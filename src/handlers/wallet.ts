...
export const createWalletHandlers = (
  ...
  const handleGenerateWallet = async (ctx: BotContext) => {
    try {
      ...
      if (!validateUserId(userId))
        return ctx.answerCbQuery("User ID not found!");
      ...
      const balance = await solanaService.getBalance(new PublicKey(publicKey));
      ...
      return sendMessageWithKeyboard(ctx, walletMessage, userId, users, {
        parse_mode: "Markdown",
      });
    } catch (error) {
  ...
  const handleCheckBalance = async (ctx: BotContext) => {
    try {
      ...
      return sendMessageWithKeyboard(ctx, formattedBalance, userId, users, {
        parse_mode: "Markdown",
      });
    } catch (error) {
  ...
  const handleTransactionHistory = async (ctx: BotContext) => {
    try {
      ...
      const formattedHistory =
        solanaService.formatTransactionHistory(transactions);

      return sendMessageWithKeyboard(ctx, formattedHistory, userId, users, {
        parse_mode: "Markdown",
      });
    } catch (error) {
  ...
  const handleSendSol = async (ctx: BotContext) => {
    try {
      ...
      return sendMessageWithKeyboard(ctx, sendMessage, userId, users, {
        parse_mode: "Markdown",
      });
    } catch (error) {
  ...
  const handleRequestAirdrop = async (ctx: BotContext) => {
    try {
      ...
      try {
        ...
        return sendMessageWithKeyboard(ctx, successMessage, userId, users, {
          parse_mode: "Markdown",
        });
      } catch (airdropError) {
        ...
        return sendMessageWithKeyboard(ctx, errorMessage, userId, users, {
          parse_mode: "Markdown",
        });
      }
    } catch (error) {
  ...
  const handleConfirmSendSol = async (ctx: BotContext) => {
    try {
      ...
      try {
        ...
        const balanceInfo = await solanaService.getBalance(
        ...
        if (balanceInfo.balance < amount) {
          ...
          return ctx.editMessageText(
            `❌ Insufficient balance. You have ${balanceInfo.balance.toFixed(6)} SOL.`,
          );
        }
  ...
  const handleCancelSendSol = async (ctx: BotContext) => {
    try {
      ...
      return ctx.editMessageText(
        "❌ **Transaction Cancelled**\n\nYou can start a new transfer anytime.",
        { parse_mode: "Markdown" },
      );
    } catch (error) {
