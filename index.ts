      ...
      try {
        ...
        if (parts.length === 2) {
          ...
          if (!recipientAddress || !amountStr) {
            return ctx.reply(
              "❌ Invalid format. Please use: <address> <amount>",
            );
          }
          ...
          try {
            ...
            if (PENDING_TRANSACTIONS[userId]) {
              return ctx.reply(
                "❌ You already have a pending transaction. Please complete or cancel it first.",
              );
            }
            ...
            if (balanceInfo.balance < amount) {
              return ctx.reply(
                `❌ Insufficient balance. You have ${balanceInfo.balance.toFixed(6)} SOL, but tried to send ${amount} SOL.`,
              );
            }
              ...
              ...Markup.inlineKeyboard([
                [
                  Markup.button.callback(
                    "✅ Confirm Send",
                    `confirm_send_sol:${userId}`,
                  ),
                  Markup.button.callback(
                    "❌ Cancel",
                    `cancel_send_sol:${userId}`,
                  ),
                ],
              ]),
            });
